...
},
{
    ...
    .width   = UXGA_WIDTH,
    .height  = UXGA_HEIGHT,
    .regs    = ov5640_capture_uxga_tbl, /*ov5640_qsxga_to_uxga_regs*/
},
{
...
},
{
    ...
    .width   = QSXGA_WIDTH,
    .height  = QSXGA_HEIGHT,
    .regs    = ov5640_capture_qsxga_tbl,// ov5640_qsxga_regs,//ov5640_capture_qsxga_tbl,     //ov5640_qsxga_regs,
},
...
#if   0 //dg add 2015-05-30,you can use this or not ,it is not must need this. codes come from oroid ov5640.c
...
static int OV5640_capture(struct regval *table, int array_size, struct v4l2_subdev *sd)
{
    ...
    write_regs(client, table, array_size); //dg change this 2015-05-30
    ...
    printk(" preview_HTS=0x%x, preview_sysclk=%ul\n", preview_HTS,preview_sysclk);
    // gain to shutter
    ...
    printk("%s shutter=0x%x, capture_VTS=0x%x\n", __func__, capture_shutter, capture_VTS);
...
static int ov5640_set_params(struct i2c_client *client, unsigned int *width, 
                             unsigned int *height, enum v4l2_mbus_pixelcode code,struct v4l2_subdev *sd)
{
    struct ov5640_priv *priv = to_ov5640(client);
    int ret = -EINVAL;
    int i;
    ...
    case RESV_UXGA: {
        reg_list = ov5640_capture_uxga_tbl;  //ov5640_qsxga_to_uxga_regs;
        list_len = ARRAY_SIZE(ov5640_capture_uxga_tbl);  //ARRAY_SIZE(ov5640_qsxga_to_uxga_regs);
        break;
    }
    case RESV_QXGA: {
        reg_list =  ov5640_capture_qxga_tbl;//ov5640_qsxga_to_qxga_regs;
        list_len =  ARRAY_SIZE(ov5640_capture_qxga_tbl); // ARRAY_SIZE(ov5640_qsxga_to_qxga_regs);
        break;
    }
    case RESV_QSXGA:
    {
        reg_list = ov5640_capture_qsxga_tbl; //ov5640_qsxga_regs;
        list_len = ARRAY_SIZE(ov5640_capture_qsxga_tbl) ; //ARRAY_SIZE(ov5640_qsxga_regs);
...
#if 0 //dg add 2015-05-30, width <=1280,height <=960  work well. codes come from oroid ov5640.c

    int result = OV5640_capture(reg_list, list_len, sd);
    //
    if (result < 0) {
        printk("%s: failed: ov5640_Capture_Return\n", __func__);
        return -EIO;
    }
#endif
    //#if 0	//remove by cym 20130807
...
#if 1 //dg add 2015-05-30,width <=1280,height <=960 work well too ,
