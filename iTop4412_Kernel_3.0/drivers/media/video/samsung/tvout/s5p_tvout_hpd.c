...
#ifdef HOTPLUG_STATE_SHOW
	ret = device_create_file(hpd_misc_device.this_device, &dev_attr_state);
	if (ret < 0)
	{
	    printk("Can't create device file 1");//goto err_create_file_1;
	}

	//ret = device_create_file(hpd_misc_device.this_device, &dev_attr_name);
	//if (ret < 0)
	//	printk("Can't create device file 2");//goto err_create_file_2;
#endif	
	last_uevent_state = -1;
...
{
#ifdef HOTPLUG_STATE_SHOW
	device_remove_file(hpd_misc_device.this_device, &dev_attr_state);
       //device_remove_file(hpd_misc_device.this_device, &dev_attr_name);	
#endif       
	return 0;
}
