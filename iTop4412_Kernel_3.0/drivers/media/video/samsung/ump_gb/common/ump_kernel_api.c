...
UMP_KERNEL_API_EXPORT void ump_dd_reference_release(ump_dd_handle memh)
{
	...
	if (0 == new_ref)
	{
		...
		rb_erase(&mem->rbnode, &rb_root_struct);
...
void _ump_ukk_msync( _ump_uk_msync_s *args )
{
	...
	_mali_osk_lock_wait(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
	if(0 != ump_descriptor_mapping_get(device.secure_id_map,
				(int)args->secure_id, (void**)&mem))
	{
		_mali_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
		DBG_MSG(1, ("Failed to mapping get in _ump_ukk_msync(). ID: %u\n", (ump_secure_id)args->secure_id));
		return;
	}
	_mali_osk_lock_signal(device.secure_id_map_lock, _MALI_OSK_LOCKMODE_RW);
