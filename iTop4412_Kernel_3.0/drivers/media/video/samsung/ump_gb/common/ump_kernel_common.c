...
struct rb_root rb_root_struct = RB_ROOT;
struct ump_dd_physical_block block_array_root = {.addr = 0, .size = 0};
struct ump_dd_mem root_node = { .block_array = &block_array_root};
...
_mali_osk_errcode_t ump_kernel_constructor(void)
{
	...
	rb_root_init(&rb_root_struct, &root_node.rbnode);
