...
static int ump_cpu_page_fault_handler(struct vm_area_struct *vma,
				      struct vm_fault *vmf);
#else
static unsigned long ump_cpu_page_fault_handler(struct vm_area_struct *vma,
...
static void ump_vma_open(struct vm_area_struct *vma)
{
	...
	DBG_MSG(4,
		("VMA open, VMA reference count incremented. VMA: 0x%08lx, reference count: %d\n",
...
static void ump_vma_close(struct vm_area_struct *vma)
{
	...
	DBG_MSG(4,
		("VMA close, VMA reference count decremented. VMA: 0x%08lx, reference count: %d\n",
		 (unsigned long)vma, new_val));
	...
	if (0 == new_val) {
		...
		DBG_MSG(4,
			("No more VMA references left, releasing UMP memory\n"));
...
_mali_osk_errcode_t _ump_osk_mem_mapregion_init(ump_memory_allocation *
						descriptor)
{
	ump_vma_usage_tracker *vma_usage_tracker;
	...
	if (NULL == vma_usage_tracker) {
		DBG_MSG(1,
			("Failed to allocate memory for ump_vma_usage_tracker in _mali_osk_mem_mapregion_init\n"));
		return -_MALI_OSK_ERR_FAULT;
	}
					       ...
					       unsigned long size)
{
	...
	retval =
	    remap_pfn_range(vma, ((u32) descriptor->mapping) + offset,
			    (*phys_addr) >> PAGE_SHIFT, size,
			    vma->
			    vm_page_prot) ? _MALI_OSK_ERR_FAULT :
	    _MALI_OSK_ERR_OK;;

	DBG_MSG(4,
		("Mapping virtual to physical memory. ID: %u, vma: 0x%08lx, virtual addr:0x%08lx, physical addr: 0x%08lx, size:%lu, prot:0x%x, vm_flags:0x%x RETVAL: 0x%x\n",
		 ump_dd_secure_id_get(descriptor->handle), (unsigned long)vma,
...
static u32 _ump_osk_virt_to_phys(ump_dd_mem *mem, u32 start, u32 address,
				 int *index)
{
	int i;
	...
	for (i = 0; i < mem->nr_blocks; i++) {
		...
		if (sum > offset) {
			*index = i;
			DBG_MSG(3,
				("_ump_osk_virt_to_phys : index : %d, virtual 0x%x, phys 0x%x\n",
				 i, address,
				 (u32) block->addr + offset - (sum -
							       block->size)));
...
static void _ump_osk_msync_with_virt(ump_dd_mem *mem, ump_uk_msync_op op,
				     u32 start, u32 address, u32 size)
{
	int start_index, end_index;
	u32 start_p, end_p;

	DBG_MSG(3,
		("Cache flush with user virtual address. start : 0x%x, end : 0x%x, address 0x%x, size 0x%x\n",
	...
	if (start_index == end_index) {
		...
		for (i = start_index; i <= end_index; i++) {
			...
			if (i == start_index) {
				if (op == _UMP_UK_MSYNC_CLEAN_AND_INVALIDATE) {
					outer_flush_range(start_p,
							  block->addr +
							  block->size);
				} else {
					outer_clean_range(start_p,
							  block->addr +
							  block->size);
				}
			} else if (i == end_index) {
				...
				if (op == _UMP_UK_MSYNC_CLEAN_AND_INVALIDATE) {
					outer_flush_range(block->addr,
							  block->addr +
							  block->size);
				} else {
					outer_clean_range(block->addr,
							  block->addr +
							  block->size);
				}
			}
...
void _ump_osk_msync(ump_dd_mem *mem, ump_uk_msync_op op, u32 start,
