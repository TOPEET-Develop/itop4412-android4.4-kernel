...
#if MMC2_SDIO_EXT_IRQ
#define MMC2_SDIO_EINT_PIN ( EXYNOS4_GPX2(5) )
#define MMC2_SDIO_EINT_IRQ (21)
...
#if MMC2_SDIO_EXT_IRQ
    if ((2 == mmc->index) && (ios->power_mode == MMC_POWER_OFF)) {
        if (0 != mmc2_sdio_ext_irq) {
            printk(KERN_INFO "%s: set mmc2 MMC_POWER_OFF disable_irq_wake & free_irq(%d)!\n", __func__, mmc2_sdio_ext_irq);
            disable_irq_wake(mmc2_sdio_ext_irq);
            free_irq(mmc2_sdio_ext_irq, host->mmc);
            mmc2_sdio_ext_irq = 0;
        }
    }
...
static irqreturn_t sdhci_sdio_ext_irq (int irq, void *dev_id)
{
    ...
    if (unlikely(test_bit(IN_SUSPEND, &mmc2_sdio_ext_irq_flags))) {
        ...
        printk(KERN_INFO "%s: irq in suspend, defer & disable_irq(%d)\n",
...
static void sdhci_setup_sdio_ext_irq (struct mmc_host *mmc)
{
    ...
    mmc2_sdio_ext_irq = gpio_to_irq(MMC2_SDIO_EINT_PIN);
    ret = request_irq(mmc2_sdio_ext_irq, sdhci_sdio_ext_irq, IRQF_TRIGGER_LOW,
        "mmc2_sdio_ext_irq", mmc);
    if (ret) {
        printk(KERN_WARNING "request_irq(%d, 0x%p, IRQF_TRIGGER_LOW) fail(%d)!!\n",
    ...
    printk(KERN_INFO "%s: using sdio_ext_irq and enable_irq_wake(%d) gpio(%d) IRQ_EINT(%d)\n",
        mmc_hostname(mmc), mmc2_sdio_ext_irq, MMC2_SDIO_EINT_PIN,
        IRQ_EINT(MMC2_SDIO_EINT_IRQ));
    return;
}
#endif /* MMC2_SDIO_EXT_IRQ */
    ...
    if (2 == mmc->index) {
        if (enable) {
            ...
            if (unlikely(test_and_clear_bit(IRQ_IN_SUSPEND, &mmc2_sdio_ext_irq_flags))) {
                smp_wmb();
                printk(KERN_INFO "%s: irq already disabled when being deferred(%d)\n",
...
static const struct mmc_host_ops sdhci_ops = {
	...
	.get_ro		= sdhci_get_ro,
//	.hw_reset	= sdhci_hw_reset, //cancel by dg
...
#if MMC2_SDIO_EXT_IRQ
    if ((2 == host->mmc->index) && !(host->flags & SDHCI_DEVICE_DEAD)) {
        if (0 != mmc2_sdio_ext_irq) {
    ...
    if (ret) {
        if ((2 == host->mmc->index) && !(host->flags & SDHCI_DEVICE_DEAD)) {
            if (0 != mmc2_sdio_ext_irq) {
                ...
                printk(KERN_INFO "%s:mmc_suspend_host fail(%d)\n",
...
int sdhci_resume_host(struct sdhci_host *host)
{
    ...
    if ((2 == host->mmc->index)
        ...
        /* force pwr reconfiguration in sdhci_init() */
        host->pwr = 0;
    }
#endif
    ...
    if ((2 == host->mmc->index) && !(host->flags & SDHCI_DEVICE_DEAD)) {
        if (0 != mmc2_sdio_ext_irq) {
            /* sdhc goes back to normal working mode, after sdhci_init() */
            clear_bit(IN_SUSPEND, &mmc2_sdio_ext_irq_flags);
            mmc_release_host(host->mmc);
            printk(KERN_INFO "%s:in_suspend--\n", mmc_hostname(host->mmc));
        }
